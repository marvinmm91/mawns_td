...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    PW.BuildingSystem.update(PW.BUILDINGS.ballista.buildTime);
    state.player.selectedTool = "repair";
    state.player.actionCooldown = 0;
    PW.Player.tryInteract();
    const upgradeStarted = building.level === 1 && PW.BuildingSystem.isUpgrading(building) && building.upgradeDuration === PW.BUILDINGS.ballista.buildTime / 2;
    PW.BuildingSystem.update(building.upgradeDuration);
    const upgradedWhenIntact = building.level === 2 && !PW.BuildingSystem.isUpgrading(building);
    building.hp = 10;
    state.player.actionCooldown = 0;
    PW.Player.tryInteract();
