...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    const setup = (gaps) => {
      ...
      state.unlockedBuildings = new Set(["palisade", "stoneWall", "steelWall"]);
      ...
      for (let y = 0; y < state.world.height; y++) {
        ...
        const wall = { id: `wall-10-${y}`, type: "palisade", x: 10, y, hp: def.maxHp, maxHp: def.maxHp, level: 1 };
  ...
  console.log("OK classic route guard", JSON.stringify({ blockedStatus: result.blockedStatus, firstBlueprint: result.firstBlueprint, secondBlueprint: result.secondBlueprint }));
})().catch((error) => {
  console.error(error);
  process.exit(1);
});
