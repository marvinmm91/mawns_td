...
(async () => {
  ...
  const result = await page.evaluate(() => {
    ...
    state.enemies = [nearest, strongest, breaker, air, shipNear];
    ...
    state.panel = "context";
    PW.UI.renderPanel();
    const contextOptions = Array.from(document.querySelectorAll('select[aria-label="Zielprioritaet"] option')).map((option) => option.value);
  ...
  const expected = { ship: "ship-near", nearest: "nearest", strongest: "strongest", breaker: "breaker", air: "air" };
  ...
  if (!result.invalidRejected || !result.loaded || result.restoredPriority !== "air" || result.options.join(",") !== "ship,nearest,strongest,breaker,air" || result.contextOptions.join(",") !== "ship,nearest,strongest,breaker,air") {
