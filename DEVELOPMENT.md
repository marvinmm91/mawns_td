...
- [x] Begegnungsskripte fuer Naechte 1 bis 10 sowie wiederkehrende, skalierende Invasionskapitel ab Nacht 11.
...
- [x] Einseitige automatische Druckerhoehung nach wiederholt sehr leichten Naechten; harte Naechte werden nicht automatisch entschärft.
...
- [x] **Begegnungsskripte und einseitiger Director** (`M`, Issue #61, Vorbereitung #66): Die ersten zehn Naechte fuehren Rollen in einer festen Reihenfolge ein. Danach rotieren Zehn-Nacht-Kapitel mit wachsendem Skriptfaktor, Schwerpunktrollen, garantierten Signaturgegnern und variablen Richtungen. Der automatische Director kann nur ansteigen und seine Werte werden nur im Entwicklungsmenue angezeigt. Betroffene Dateien: `js/data/waves.js`, `js/systems/spawning.js`, `js/systems/autobalance.js`, `js/systems/save.js`, `js/ui/panels.js`, `GAME_DESIGN.md`, `tests/`. Akzeptanz: Nacht 1-10, 11-60 und die Fortsetzung danach sind reproduzierbar; alte Saves bleiben lauffaehig; kein Nachtbericht oder Status zeigt den internen Director.

- [x] **Wellenabschluss statt Nacht-Timer** (`M`, Issue #59): Regulaere Nachtwellen beginnen sofort und enden erst nach dem letzten zugeordneten Wellengegner. Der HUD-Timer zeigt waehrenddessen den aktiven Wellenzustand; die finale Startsequenz bleibt zeitbasiert. Betroffene Dateien: `js/systems/dayNight.js`, `js/systems/spawning.js`, `js/entities/enemies.js`, `js/systems/save.js`, `js/ui/*`. Akzeptanz: Stationaere Horden halten die Nacht nicht auf, alte laufende Saves erhalten eine Wellenzuordnung und der Morgen beginnt erst nach dem letzten Wellenkill.
...
- [x] **Classic Mode** (`M`, Issue #54): Regulaere Bodengegner ignorieren Mauer, Palisade und Turm bei freiem oder alternativem Weg. Vollstaendige Sperren duerfen gebaut werden; ohne Weg greifen Bodengegner das gezielte Durchbruchziel mit zwanzigfachem Classic-Strukturschaden an, bis wieder ein Weg offen ist. Wrack, Luftgegner und stationaere Wachen sind ausgeschlossen. Classic bietet nur die Palisade neu an; sie bleibt bewusst nicht ausbaubar, bestehende andere Mauern bleiben kompatibel. Betroffene Dateien: `js/entities/enemies.js`, `js/systems/pathfinding.js`, `js/systems/building.js`, `js/config.js`, `GAME_DESIGN.md`, `tests/`. Akzeptanz: Freie Wege bleiben frei von Strukturangriffen; vollständige Einsperrungen sind baubar und werden zuverlässig aufgebrochen.
...
- [x] **Schwierigkeitsprofile und Bedrohungsprognose** (`M`, Issue #43): Fuenf vor dem Start waehlbare Stufen beeinflussen ausschliesslich Grundbudget und Grenzen des vorhandenen Bedrohungsdirektors; Stufe 3 entspricht den bisherigen Standardwerten. Die Stufe bleibt im Status sichtbar, konkrete Directorwerte erscheinen nur im Entwicklungsmenue. Das Profil bleibt im Spielstand erhalten; alte Spielstaende migrieren auf Standard. Betroffene Dateien: `js/config.js`, `js/state.js`, `js/systems/autobalance.js`, `js/systems/save.js`, `js/bootstrap.js`, `js/ui/panels.js`, `tests/`. Akzeptanz: Alle Profile liefern reproduzierbar abgestufte Budgets; Standard bleibt unveraendert; Auswahl und Save/Load funktionieren.
