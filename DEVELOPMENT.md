...
- [x] **Classic Mode** (`M`, Issue #54): Regulaere Bodengegner ignorieren Mauer, Palisade und Turm bei freiem oder alternativem Weg. Die Baupruefung verhindert dabei schon beim Setzen blockierender Mauern, Tuerme und Blaupausen jede vollstaendige Sperre zwischen moeglichen Spawnraendern beziehungsweise laufenden Bodengegnern und dem Wrack. Nur als Absicherung fuer alte oder fehlerhafte Blockaden greift das gezielte Durchbruchziel mit zehnfachem Schaden an; Wrack, Luftgegner und stationaere Wachen sind ausgeschlossen. Classic bietet nur die Palisade neu an, bestehende andere Mauern bleiben kompatibel. Betroffene Dateien: `js/entities/enemies.js`, `js/systems/pathfinding.js`, `js/systems/building.js`, `js/config.js`, `GAME_DESIGN.md`, `tests/`. Akzeptanz: Freie Wege bleiben frei von Strukturangriffen und vollstaendige Einsperrungen sind nicht platzierbar.
