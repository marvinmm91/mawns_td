...
PW.Autobalance = {
  threatBudgetForNight(night) {
    ...
    const base = cfg.baseThreatBudget + (night - 1) * cfg.budgetGrowth + Math.pow(Math.max(0, night - 1), 1.22) * 0.9;
    const drift = PW.Utils.clamp(PW.state.balance.drift, cfg.maxNegativeDrift, cfg.maxPositiveDrift);
    const budget = base * (1 + drift);
  ...
  evaluateNight() {
    ...
    if (hard) {
      ...
      state.balance.dropBonus = Math.min(0.22, state.balance.dropBonus + 0.06);
  ...
  diagnose(stats, damageRatio, hpRatio, avgKillDistance) {
    ...
    if (damageRatio > 0.35 || hpRatio < 0.35) lines.push("Naechste Nacht wird leicht entschaerft und Schrottdrops steigen.");
    else if (damageRatio < 0.05 && avgKillDistance > PW.state.world.tileSize * 9) lines.push("Verteidigung war sehr stark. Der naechste Angriff kann breiter werden.");
